...
@section('title') @lang('Activity - Home') @endsection
@section('css')
        ...
        <div class="card mb-0">
            <div class="card-body">
                 <div class="row align-items-center">
                     <div class="col-md-6">
                         <div class="mb-3">
                             <h5 class="card-title">Activities List <span class="text-muted fw-normal ms-2">{{(count($activities))}}</span></h5>
                         </div>
                     </div>
                     ...
                     <table class="table align-middle datatable dt-responsive table-check nowrap" style="border-collapse: collapse; border-spacing: 0 8px; width: 100%;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th scope="row">
                                    <div class="form-check font-size-16">
                                        <input type="checkbox" class="form-check-input" id="contacactivitycheck1">
                                        <label class="form-check-label" for="contacactivitycheck1"></label>
                                    </div>
                                </th>
                                ...
                                <td>
                                    ...
                                    <a href="#" class="text-primary" data-bs-toggle="modal" data-bs-target="#bannerModal-{{ $activity->id }}">
                                        Lihat Gambar Activitas
                                    </a>

                                    <!-- Modal Bootstrap -->
                                    <div class="modal fade" id="bannerModal-{{ $activity->id }}" tabindex="-1" aria-labelledby="bannerModalLabel-{{ $activity->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="bannerModalLabel-{{ $activity->id }}">Gambar Aktivitas</h5>
                                                ...
                                                <div class="modal-body text-center">
                                                    ...
                                                    @if ($activity->images->isNotEmpty())
                                                        @foreach ($activity->images as $image)
                                                            <img src="{{ asset('storage/'.$image->image) }}" alt="Activity Image" class="img-fluid rounded mb-2">
                                                        @endforeach
                                                    @endif
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            <li>
                                                <a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#editActivitiesModal-{{ $activity->id }}" href="#">
                                                    Edit
                                                </a>
                                            </li>
                                            <li>
                                                <a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalDelete-{{ $activity->id }}" href="#">
                                                    Delete
                                                </a>
                                            </li>
                                        </ul>
                                ...
                                <div class="modal fade" id="editActivitiesModal-{{ $activity->id }}" tabindex="-1" aria-labelledby="editSActivityModalLabel-{{ $activity->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            ...
                                            <form action="{{ route('adminIndex.activities.update', $activity->id) }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                <div class="modal-body">
                                                    ...
                                                    <!-- title -->
                                                    <div class="mb-3">
                                                        <label for="title-{{ $activity->id }}" class="form-label">title</label>
                                                        <input type="text" name="title" id="title-{{ $activity->id }}" class="form-control" value="{{ $activity->title ?? '' }}">
                                                    </div>

                                                    <!-- Description -->
                                                    <div class="mb-3">
                                                        <label for="description-{{ $activity->id }}" class="form-label">Description</label>
                                                        <textarea name="description" id="description-{{ $activity->id }}" class="form-control" rows="3">{{ $activity->description ?? '' }}</textarea>
                                                    </div>

                                                    <!-- Location -->
                                                    <div class="mb-3">
                                                        <label for="location-{{ $activity->id }}" class="form-label">Location</label>
                                                        <input type="text" name="location" id="location-{{ $activity->id }}" class="form-control" value="{{ $activity->location ?? '' }}">
                                                    </div>

                                                    <!-- Foto Profil -->
                                                    <div class="mb-3">
                                                        <label for="images-{{ $activity->id }}" class="form-label">Gambar</label>
                                                        <input type="file" name="images" id="images-{{ $activity->id }}" class="form-control">
                                ...
                                <div class="modal fade" id="modalDelete-{{ $activity->id }}" tabindex="-1" aria-labelledby="modalDeleteLabel-{{ $activity->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="modalDeleteLabel-{{ $activity->id }}">Konfirmasi Hapus</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                Apakah Anda yakin ingin menghapus <strong>{{ $activity->name }}</strong>? Tindakan ini tidak dapat dibatalkan.
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tidak</button>
                                                <form action="{{ route('adminIndex.activities.destroy', $activity->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                <!-- end of modal untuk menghapus activity -->
