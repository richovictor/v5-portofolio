...
@section('title') @lang('Certificates - Home') @endsection
@section('css')
        ...
        <div class="card mb-0">
            <div class="card-body">
                 <div class="row align-items-center">
                     <div class="col-md-6">
                         <div class="mb-3">
                             <h5 class="card-title">Certificates List <span class="text-muted fw-normal ms-2">{{(count($certificates))}}</span></h5>
                         </div>
                     </div>
                     ...
                     <table class="table align-middle datatable dt-responsive table-check nowrap" style="border-collapse: collapse; border-spacing: 0 8px; width: 100%;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th scope="row">
                                    <div class="form-check font-size-16">
                                        <input type="checkbox" class="form-check-input" id="contaccertificatecheck1">
                                        <label class="form-check-label" for="contaccertificatecheck1"></label>
                                    </div>
                                </th>
                                <td>{{$certificate->user->name}}</td>
                                <td>{{$certificate->title}}</td>
                                <td>{{$certificate->agency}}</td>
                                <td>{{$certificate->description}}</td>
                                <td>{{$certificate->location}}</td>
                                <td>{{$certificate->created_at}}</td>
                                <td>
                                    <!-- Teks untuk membuka modal -->
                                    <a href="#" class="text-primary" data-bs-toggle="modal" data-bs-target="#bannerModal-{{ $certificate->id }}">
                                        Lihat Gambar Sertifikat
                                    </a>

                                    <!-- Modal Bootstrap -->
                                    <div class="modal fade" id="bannerModal-{{ $certificate->id }}" tabindex="-1" aria-labelledby="bannerModalLabel-{{ $certificate->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="bannerModalLabel-{{ $certificate->id }}">Gambar Aktivitas</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body text-center">
                                                    <!-- Gambar Banner -->
                                                    @if ($certificate->images->isNotEmpty())
                                                        @foreach ($certificate->images as $image)
                                                            <img src="{{ asset('storage/'.$image->image) }}" alt="certificate Image" class="img-fluid rounded mb-2">
                                                        @endforeach
                                                    @endif
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            <li>
                                                <a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#editActivitiesModal-{{ $certificate->id }}" href="#">
                                                    Edit
                                                </a>
                                            </li>
                                            <li>
                                                <a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modalDelete-{{ $certificate->id }}" href="#">
                                                    Delete
                                                </a>
                                            </li>
                                        </ul>
                                ...
                                <div class="modal fade" id="editActivitiesModal-{{ $certificate->id }}" tabindex="-1" aria-labelledby="editScertificateModalLabel-{{ $certificate->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            ...
                                            <form action="{{ route('adminIndex.certificates.update', $certificate->id) }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                <div class="modal-body">
                                                    ...
                                                    <!-- title -->
                                                    <div class="mb-3">
                                                        <label for="title-{{ $certificate->id }}" class="form-label">title</label>
                                                        <input type="text" name="title" id="title-{{ $certificate->id }}" class="form-control" value="{{ $certificate->title ?? '' }}">
                                                    </div>

                                                    <!-- Description -->
                                                    <div class="mb-3">
                                                        <label for="agency-{{ $certificate->id }}" class="form-label">Agency</label>
                                                        <textarea name="agency" id="agency-{{ $certificate->id }}" class="form-control" rows="3">{{ $certificate->description ?? '' }}</textarea>
                                                    </div>

                                                    <!-- Description -->
                                                    <div class="mb-3">
                                                        <label for="description-{{ $certificate->id }}" class="form-label">Description</label>
                                                        <textarea name="description" id="description-{{ $certificate->id }}" class="form-control" rows="3">{{ $certificate->description ?? '' }}</textarea>
                                                    </div>

                                                    <!-- Location -->
                                                    <div class="mb-3">
                                                        <label for="location-{{ $certificate->id }}" class="form-label">Location</label>
                                                        <input type="text" name="location" id="location-{{ $certificate->id }}" class="form-control" value="{{ $certificate->location ?? '' }}">
                                                    </div>

                                                    <!-- Gambar -->
                                                    <div class="mb-3">
                                                        <label for="images-{{ $certificate->id }}" class="form-label">Gambar</label>
                                                        <input type="file" name="images" id="images-{{ $certificate->id }}" class="form-control">
                                ...
                                <div class="modal fade" id="modalDelete-{{ $certificate->id }}" tabindex="-1" aria-labelledby="modalDeleteLabel-{{ $certificate->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="modalDeleteLabel-{{ $certificate->id }}">Konfirmasi Hapus</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                Apakah Anda yakin ingin menghapus <strong>{{ $certificate->name }}</strong>? Tindakan ini tidak dapat dibatalkan.
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tidak</button>
                                                <form action="{{ route('adminIndex.certificates.destroy', $certificate->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                <!-- end of modal untuk menghapus certificate -->
